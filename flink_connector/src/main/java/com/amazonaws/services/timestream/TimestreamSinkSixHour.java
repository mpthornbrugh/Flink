 ...
 */
public class TimestreamSinkSixHour extends RichSinkFunction<AggregationRecord> implements CheckpointedFunction {
    private static final Logger LOG = LoggerFactory.getLogger(TimestreamSinkSixHour.class);
    ...
    public TimestreamSinkSixHour(String region, String databaseName, String tableName, int batchSize) {
        this.region = region;
    ...
    public void invoke(AggregationRecord value, Context context) throws Exception {
        ...
        LOG.info("MICHAEL Inside TimestreamSinkSixHour");
        ...
        for (Map.Entry mapElement : bufferedRecords.entrySet()) {
            ...
            for (BufferedRecord element : records) {
                int hour_window = element.getSixHourWindow();
                ...
                for (int i = 0; i < agg_times.size() - 1; i++) {
                    ...
                    for (BufferedRecord current_rec : current_buff_recs) {
                        if (!values_set) {
                            ...
                            hour_start_ts = current_rec.getSixHourWindow() * 21600;
                            values_set = true;
                        }
        ...
        if (aggregate_records.size() > 0) {
            ...
            for (Result agg_rec : aggregate_records) {
                ...
                Dimension time_window_dim = new Dimension().withName("time_window").withValue("6");
